...
#define rep(i, j) f(i, 0, j, 1)
#define rrep(i, j) rf(i, j, 0, 1)
...
inline void solve() {
    ...
    */
    ...
    char arr[n][n];
    cin >> s;
    int ones = count(s.begin(), s.end(), '1'), twos = n - ones;
    if (twos && twos < 3) {
        no();
        return;
    }
    int first2 = -1, second2 = -1, third2 = -1;
        ...
        if (s[i] == '1')
            ...
            for (int j = i + 1; j < n; j++) {
                if (s[j] == '1') {
                    ...
                    if ((i == first2 && j == second2) || (i == second2 && j == third2)) {
                        arr[i][j] = '+';
                        arr[j][i] = '-';
                    } else {
                        arr[i][j] = '-';
